...
function createTextPlaneGeom( textPlane, width, height ){
	...
	var geom = new THREE.PlaneGeometry( width, height, 3, 1 );
	return geom;
...
function createCompoundPlaneMaterial( geom ){
	...
	for ( var i = 0; i < geom.faces.length; i++ ){
		...
		else if ( !_Math.isEven(i) ) {
			geom.faces[i].materialIndex = ( i - 1 );
		}
	} 	
	...
	for(var i = 0; i < geom.faces.length; i++){      
		...
		if ( !_Math.isEven(i) ){		
			geom.faceVertexUvs[0].push( vertices270Deg );
		}
		else if ( _Math.isEven(i) ) {
			geom.faceVertexUvs[0].push( vertices90Deg );
		}
...
var textPlanes = {
	rec1b64: new textPlane( "Hello!", 12, 8, 100, { x: -4, y: 20, z: 0 }, { r: 11, g: 18, b: 70 }, { r: 255, g: 0, b: 0 } ),
